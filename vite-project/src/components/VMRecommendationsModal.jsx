...
const VMRecommendationsModal = ({ isOpen, onClose, vmName, timeRangeDays = 7, selectedDate = 'today' }) => {
  ...
  const getCategoryConfig = (category) => {
    switch (category) {
      ...
      case 'cost_optimization':
        return { icon: DollarSign, color: 'text-green-600 dark:text-green-400' };
      case 'sustainability':
        return { icon: Zap, color: 'text-yellow-600 dark:text-yellow-400' };
      case 'optimization':
        return { icon: Settings, color: 'text-purple-600 dark:text-purple-400' };
      default:
  ...
  const fetchRecommendations = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8000/api/recommendations/vm/${encodeURIComponent(vmName)}?${queryParams}`);
